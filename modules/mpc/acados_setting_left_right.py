...
def acados_setting_left_right(ind):
    ...
    # Q(3, 3) = 50  # heading 가중치 (높을수록 목표 방향 준수 능력이 높아짐, 낮을수록 많이 틀어져도 됨)
    # Q(4, 4) = 300  # 속도 가중치 → 높으면 속도 유지 중시
    # Q(2, 2) = 0.05  # y 위치 가중치 → 높으면 경로 이탈 억제

    QN = Q * 5
    Rd = np.diag([500, 100])

    # Rd(1, 1) = 500  # 가속도 억제 → 높으면 부드러운 가속
    # Rd(2, 2) = 100  # 각가속도 억제 → 높으면 부드러운 선회
