"""
GP-BPL + MPC 운항 스타일 학습 테스트 v3
- GP-BPL: Laplace Approximation 기반 정확한 posterior 계산
- utility  = GP posterior mean (MAP 추정)
- variance = GP posterior variance (정확한 predictive variance)
- reasoning: pseudo-observation으로 GP에 직접 통합
"""
...
# ──────────────────────────────────────────
# GP-BPL v3 — Laplace Approximation 기반 정확한 구현
# ──────────────────────────────────────────
class GPBPL:
    """
    정확한 GP-BPL 구현 (gpytorch 없이 scipy로 구현)

    수학적 구조:
      Prior  : f ~ GP(0, K)   — K: RBF kernel
      Likelihood: P(A>B|f) = σ(f_A - f_B)   — Bradley-Terry 모델
      Posterior: p(f|D) ∝ P(D|f) · p(f)    — Laplace approximation
      Predictive: p(f*|D) = N(μ*, σ*²)

    기존 근사 대비 개선점:
      - utility: kernel 합산 → 정확한 GP posterior mean (MAP)
      - variance: 근사 수식 → 정확한 GP posterior variance
      - reasoning: pseudo-observation으로 GP에 직접 통합
    """

    def __init__(self, dim, bounds, length_scale=1.2,
                 kernel_var=1.0, noise=0.1):
        self.dim    = dim
        self.bounds = bounds
        self.ls     = length_scale      # RBF kernel length scale
        self.kvar   = kernel_var        # kernel output variance
        self.noise  = noise             # likelihood noise (Laplace regularization)

        # 관측 데이터
        self.X      = []   # 모든 관측 지점 (normalized)
        self.pairs  = []   # (idx_A, idx_B, chose_A) — 쌍비교
        self.pseudo = []   # (idx, target_val, conf) — reasoning pseudo-obs

        # Laplace posterior (업데이트 후 캐시)
        self._f_map = None   # MAP 추정값 f*
        self._K     = None   # 커널 행렬
        self._K_inv = None   # K^{-1}
        self._W     = None   # -∇² log p(D|f) at f_map (Hessian)
        self._dirty = True   # 새 관측 추가 시 재계산 필요
    ...
    def _kernel(self, X1, X2):
        """RBF kernel: k(x1,x2) = kvar * exp(-||x1-x2||² / 2ls²)"""
        X1 = np.atleast_2d(X1)
        X2 = np.atleast_2d(X2)
        diff = X1[:, None, :] - X2[None, :, :]
        return self.kvar * np.exp(
            -0.5 * np.sum(diff ** 2, axis=-1) / self.ls ** 2
        )

    def _k_vec(self, x, X):
        """k(x, X): (n,) 벡터"""
        return self._kernel(x[None], X)[0]

    # ── Laplace Approximation ───────────────
    def _log_likelihood(self, f):
        """log P(D|f) = Σ log σ(±(f_A - f_B)) + Σ pseudo obs"""
        ll = 0.0
        for iA, iB, chose_A in self.pairs:
            diff = f[iA] - f[iB]
            ll += np.log(sigmoid(diff) + 1e-12) if chose_A \
                  else np.log(sigmoid(-diff) + 1e-12)
        for idx, target, conf in self.pseudo:
            ll -= conf * 0.5 * (f[idx] - target) ** 2
        return ll
    ...
    def _grad_ll(self, f):
        ...
        for iA, iB, chose_A in self.pairs:
            ...
            if chose_A:
                ...
                g[iA] -= s;        g[iB] += s
        for idx, target, conf in self.pseudo:
            g[idx] += conf * (target - f[idx])
        return g

    def _hess_diag_ll(self, f):
        """diag(-∇² log P(D|f)) — Laplace에서 W 행렬의 대각"""
        w = np.zeros(len(f))
        for iA, iB, _ in self.pairs:
            s = sigmoid(f[iA] - f[iB])
            lam = s * (1 - s)
            w[iA] += lam;  w[iB] += lam
        for idx, target, conf in self.pseudo:
    ...
    def _fit(self):
        """Laplace approximation: MAP 추정 + posterior 파라미터 계산"""
        n = len(self.X)
        if n == 0:
            self._dirty = False
            return
        ...
        # MAP: argmin { 0.5 f^T K^{-1} f - log P(D|f) }
        f0 = np.zeros(n)
        def neg_log_post(f):
            return 0.5 * f @ K_inv @ f - self._log_likelihood(f)
        def neg_grad(f):
            return K_inv @ f - self._grad_ll(f)

        res = minimize(neg_log_post, f0, jac=neg_grad,
                       method='L-BFGS-B',
        ...
        self._K     = K
        self._K_inv = K_inv
        self._W     = np.diag(self._hess_diag_ll(res.x))  # W = diag(w)
        self._dirty = False

    # ── Public API ─────────────────────────
    def _add_point(self, x_norm):
        """관측 지점 추가 후 인덱스 반환"""
    ...
    def update_pairwise(self, tA, tB, chose_A):
        """쌍비교 관측 추가"""
        iA = self._add_point(self._norm(tA))
        iB = self._add_point(self._norm(tB))
        self.pairs.append((iA, iB, chose_A))
        self._dirty = True

    def update_reasoning(self, parsed):
        """reasoning 신호를 pseudo-observation으로 추가"""
        for i, name in enumerate(THETA_NAMES):
            info = parsed[name]
            if info["confidence"] > 0.0:
                # direction을 [-1, 1] → 실제 f 값으로 스케일
                target = (info["direction"] - 0.5) * 2.0
                # pseudo-observation 지점: 해당 파라미터 축 방향
                x_pseudo = np.full(self.dim, 0.5)
                x_pseudo[i] = info["direction"]
                idx = self._add_point(x_pseudo)
                self.pseudo.append((idx, target, info["confidence"]))
        self._dirty = True
    ...
    def utility(self, theta):
        """GP posterior mean μ*(θ) — 정확한 predictive mean"""
        self._ensure_fit()
        if not self.X:
            return 0.0
        x = self._norm(theta)
        k_star = self._k_vec(x, np.array(self.X))
        return float(k_star @ self._K_inv @ self._f_map)

    def variance(self, theta):
        """GP posterior variance σ*²(θ) — 정확한 predictive variance

        Laplace approximation:
          Σ = (K^{-1} + W)^{-1}
          σ*²(θ) = k(θ,θ) - k_*^T (K + W^{-1})^{-1} k_*
        """
        self._ensure_fit()
        if not self.X:
            return float(self.kvar)
        x      = np.array(self.X)
        k_star = self._k_vec(self._norm(theta), x)
        k_self = float(self.kvar)

        # (K + W^{-1})^{-1} via Woodbury / direct solve
        W_diag = np.diag(self._W)
        # W가 모두 0이면 (관측 없음) K^{-1}만 사용
        if np.all(W_diag < 1e-10):
            v = self._K_inv @ k_star
        else:
            W_inv_diag = np.where(W_diag > 1e-10, 1.0 / W_diag, 1e10)
            M = self._K + np.diag(W_inv_diag)
            try:
                c, low = cho_factor(M)
                v = cho_solve((c, low), k_star)
            except Exception:
                v = np.linalg.solve(M, k_star)

        var = k_self - float(k_star @ v)
        return float(max(1e-6, var))

    def mean_variance(self, n=100):
        """전체 파라미터 공간의 평균 posterior variance — 수렴 지표"""
    ...
    def acquisition(self, n_cand=400):
        """Maximum Variance acquisition: σ*²가 가장 큰 두 지점 선택"""
    ...
    def param_variances(self):
        """각 파라미터 축별 평균 posterior variance (시각화용)"""
        result = []
        for i in range(self.dim):
            samps = np.linspace(self.bounds[i, 0], self.bounds[i, 1], 20)
            vlist = []
            for s in samps:
                t = (self.bounds[:, 0] + self.bounds[:, 1]) / 2
                t[i] = s
                vlist.append(self.variance(t))
            result.append(float(np.mean(vlist)))
        return result
...
def main():
    ...
    gp  = GPBPL(dim=4, bounds=THETA_BOUNDS, length_scale=1.2,
                kernel_var=1.0, noise=0.1)
    mpc = MPCSim()
    ...
    fig_it.suptitle("GP-BPL v3 (Laplace Approx) Iterations",
